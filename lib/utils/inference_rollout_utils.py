...
def clip_xywh_boxes_to_image_bounds(boxes_xywh: torch.Tensor, image_height: int, image_width: int,
                                    min_size: float = 1.0) -> torch.Tensor:
    x1 = boxes_xywh[:, 0]
    y1 = boxes_xywh[:, 1]
    bw = boxes_xywh[:, 2]
    bh = boxes_xywh[:, 3]
    ...
    x1 = x1.clamp(min=0.0, max=max(0.0, image_width - min_size))
    x2 = x2.clamp(min=min_size, max=float(image_width))
    y1 = y1.clamp(min=0.0, max=max(0.0, image_height - min_size))
    y2 = y2.clamp(min=min_size, max=float(image_height))

    bw = (x2 - x1).clamp(min=min_size)
    bh = (y2 - y1).clamp(min=min_size)
...
def sample_target_crop_from_chw_tensor(image_chw: torch.Tensor, target_bb_xywh: torch.Tensor,
                                       search_area_factor: float, output_sz: int) -> Tuple[torch.Tensor, float]:
    image_hwc = image_chw.detach().cpu().permute(1, 2, 0).numpy()
    crop_hwc, resize_factor, _ = sample_target(
        image_hwc,
        target_bb_xywh.detach().cpu(),
        search_area_factor=search_area_factor,
        output_sz=output_sz,
    )
    crop_chw = torch.from_numpy(crop_hwc).to(device=image_chw.device, dtype=image_chw.dtype).permute(2, 0, 1)
    return crop_chw, float(resize_factor)
...
def sample_target_crops_from_bchw_tensor(images_bchw: torch.Tensor,
    ...
    """Batched tensor version of sample_target for rollout crops."""
    ...
    batch_size, _, image_h, image_w = images_bchw.shape
    device = images_bchw.device
    dtype = images_bchw.dtype

    boxes = target_boxes_xywh.detach().to(device=device, dtype=dtype)
    x = boxes[:, 0]
    y = boxes[:, 1]
    ...
    x1 = torch.round(x + 0.5 * box_w - 0.5 * crop_sz)
    y1 = torch.round(y + 0.5 * box_h - 0.5 * crop_sz)

    coords = torch.arange(int(output_sz), device=device, dtype=dtype)
    src_x = x1[:, None] + (coords[None, :] + 0.5) / resize_factors[:, None] - 0.5
    src_y = y1[:, None] + (coords[None, :] + 0.5) / resize_factors[:, None] - 0.5
    ...
    grid_x = 2.0 * (grid_x + 0.5) / float(image_w) - 1.0
    grid_y = 2.0 * (grid_y + 0.5) / float(image_h) - 1.0
