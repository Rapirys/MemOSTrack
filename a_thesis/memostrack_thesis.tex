...
\item if CE is not enabled at layer \(l\), the sequence is left unchanged.
\end{itemize}
...
\texttt{torch.no\_grad()}, so the optimized target filter is treated as a pseudo-label rather than as a fully differentiable
...
\item
  \(w_{\text{match}}\) is \texttt{FILTER\_MATCH\_WEIGHT}.
\item
