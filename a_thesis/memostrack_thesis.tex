...
\item \(P\) is the patch size, assuming that \(H\) and \(W\) are divisible by \(P\);
\item \(N\) is the number of patches;
\item \(x_i \in \mathbb{R}^{P^2C}\) is the flattened vector of the \(i\)-th patch;
\item \(X_p\) is the matrix containing all flattened patch vectors.
}

The flattened patches are projected into a \(D\)-dimensional embedding space, and
...
\end{equation}
\FormulaNotation{
\item \(E_X \in \mathbb{R}^{N \times D}\) is the patch-embedding sequence;
\item \(W_E \in \mathbb{R}^{P^2C \times D}\) is the learnable patch-projection matrix;
\item \(b_E \in \mathbb{R}^{D}\) is the learnable projection bias, broadcast across all patches;
\item \(D\) is the token embedding dimension;
\item \(P_{\mathrm{pos}} \in \mathbb{R}^{N \times D}\) is the positional embedding;
\item \(Z_0 \in \mathbb{R}^{N \times D}\) is the initial token sequence passed to the transformer encoder.
}
...
\item \(A_l\) is the normalized token sequence;
\item \(\operatorname{MSA}(\cdot)\) denotes Multi-Head Self-Attention;
\item \(U_l\) is the token sequence after self-attention and residual addition.
}
...
\item \(\operatorname{CE}_l(\cdot)\) denotes the candidate elimination operation at layer \(l\);
\item \(U_l\) is the unpruned token sequence from the attention-and-residual step.
}
...
\item \(\bar{U}_l\) is the input to the MLP part of the encoder layer;
\item \(\operatorname{MLP}(\cdot)\) is the feed-forward network;
\item \(\operatorname{LN}(\cdot)\) denotes Layer Normalization.
}
...
\end{equation}
\FormulaNotation{
\item \(A\) is the normalized input sequence to the attention operation;
\item \(h\) is the attention-head index;
\item \(Q_h\), \(K_h\), and \(V_h\) are the query, key, and value matrices for head \(h\);
\item \(W_Q^h\), \(W_K^h\), and \(W_V^h\) are learnable projection matrices for head \(h\);
\item \(d_h\) is the feature dimension of one attention head;
\item \(n_h\) is the number of attention heads;
\item \(W_O\) is the output projection matrix after concatenating all heads.
}
...
\end{equation}
\FormulaNotation{
\item \(z \in \mathbb{R}^{D}\) is one token vector;
\item \(W_1\), \(W_2\), \(b_1\), and \(b_2\) are learnable MLP parameters;
\item \(\operatorname{GELU}(\cdot)\) is the activation function used inside the feed-forward block.
}
\\
...
\end{equation}
\FormulaNotation{
\item \(T\) is the template crop from the first annotated frame;
\item \(S_t\) is the search crop from frame \(t\);
\item \(\operatorname{PatchEmbed}(\cdot)\) denotes patch flattening followed by linear projection;
\item \(P_T\) and \(P_S\) are positional embeddings for the template and search tokens;
\item \(T_0 \in \mathbb{R}^{N_T \times D}\) is the initial template-token sequence;
\item \(S_{t,0} \in \mathbb{R}^{N_S \times D}\) is the initial search-token sequence;
\item \([\cdot,\cdot]\) denotes concatenation along the token dimension;
\item \(Z_{t,0}^{\mathrm{track}}\) is the combined OSTrack input sequence for frame \(t\).
}
...
\item \(Z_{t,L}^{\mathrm{track}}\) is the final backbone output for frame \(t\);
\item \(\operatorname{ViTBackbone}(\cdot)\) denotes the full ViT backbone mapping;
\item \(T_L^{(t)}\) denotes the processed template-token representation while tracking frame \(t\);
\item \(S_{t,L}\) denotes the processed search-token representation for frame \(t\).
}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{vit_example.png}
\end{figure}
...
\end{equation}
\FormulaNotationWith{Here:}{
\item \(X'_l\) is the output token sequence after the attention sublayer at layer \(l\);
\item \(X_{l-1}\) represents the input token sequence, including search, template, and memory tokens;
\item \(\operatorname{LN}(\cdot)\) denotes Layer Normalization;
\item \(\operatorname{MSA}(\cdot)\) denotes Multi-Head Self-Attention.
}
...
\section{Variant B: GRU-based memory update and two-stage architecture}\label{variant-b-gru-based-memory-update-and-two-stage-architecture}
