...
\begin{document}
    ...
    The main limitation of this thesis is that the final experiment tests all proposed
    changes together. The final MemOSTrack model combines memory tokens, a GRU-based memory
    update, a modified sequence-based training loop, dynamic search cropping, auxiliary
    memory losses, and template blurring. Because these changes were not tested separately,
    ...
    In experimental evaluations on the GOT-10k benchmark, the final MemOSTrack-256 + CE model achieved an AO of 0.729, an SR0.50 of 0.823, and an SR0.75 of 0.693. These results show that the complete MemOSTrack configuration improves over the reported same-resolution OSTrack-256 + CE baseline. However, because several modifications were evaluated jointly, future ablation studies are required to determine the individual contributions of the memory tokens, the GRU mechanism, and the dynamic training pipeline to the overall performance gain.
