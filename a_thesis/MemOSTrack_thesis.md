...
| Deformation | Person, animal, or flexible object changes shape | Appearance no longer matches the initial template
exactly. |
...
OSTrack is a real-time single-object visual tracker introduced by Ye et al. in *Joint Feature Learning and Relation
...
Specifically, the architecture must decouple the *proposal* of new memory from the actual *update* of the memory state,
...
$$
L_t = \frac{1}{|\mathcal{S}_t|} \sum_{j \in \mathcal{S}_t} \lVert x_j * f_t - y_j \rVert_2^2 + \lambda \lVert f_t
\rVert_2^2.
$$
